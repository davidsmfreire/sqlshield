...
fn schema_parses_multiple_tables() {
    ...
    ";
    let tables = load_schema(schema, "sql", Dialect::default()).unwrap();
...
fn missing_file_returns_io_error() {
    let err = load_schema_from_file(
        std::path::Path::new("/nonexistent/schema.sql"),
        Dialect::default(),
    )
    .unwrap_err();
...
fn file_without_extension_returns_missing_extension_error() {
    ...
    let err = load_schema_from_file(&path, Dialect::default()).unwrap_err();
